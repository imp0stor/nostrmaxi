...
export function HashtagTable({ hashtags, onTagClick }: { hashtags: HashtagStat[]; onTagClick?: (tag: string) => void }) {
  ...
  return (
    <div className="space-y-2">
      {hashtags.map((tag, i) => (
        <button key={tag.tag} type="button" onClick={() => onTagClick?.(tag.tag)} className="w-full flex items-center gap-3 hover:bg-gray-800/40 rounded px-1 py-1 transition-colors text-left">
